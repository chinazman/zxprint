   ...
   const reconstitutedColumns = TableExcelHelper.reconsitutionTableColumnTree(columns.filter(column => !TableExcelHelper.isFooterRow(column)));
    ...
    const createRow = (layerIndex) => {
      ...
      reconstitutedColumns[layerIndex].filter(column => column.checked).forEach(column => {
        ...
        column.hFontWeight && cell.css("font-weight", column.hFontWeight).attr("hfontweight", "1");
        column.hFontSize && cell.css("font-size", column.hFontSize + "pt").attr("hfontsize", "1");
        column.hColor && cell.css("color", column.hColor).attr("hcolor", "1");
